...
use crate::{serial_println, KERNEL_INFO};

pub static FRAME_ALLOCATOR: Once<Mutex<BootInfoFrameAllocator>> = Once::new();
pub static KERNEL_MAPPER: Once<Mutex<OffsetPageTable<'static>>> = Once::new();

pub fn init_memory() {
    let kernel_info = KERNEL_INFO.get().unwrap();
    let frame_allocator = unsafe { BootInfoFrameAllocator::new(&kernel_info.memory_map) };
...
pub fn allocate_page_table(
    ...
    let physical_memory_offset = KERNEL_INFO.get().unwrap().physical_memory_offset;
    ...
    unsafe {
        ...
        let kernel_l3_table = ((kernel_l4_table[0].addr().as_u64() + physical_memory_offset) as *const PageTable).as_ref().unwrap();
...
pub unsafe fn init() -> OffsetPageTable<'static> {
    OffsetPageTable::new(
        ...
        VirtAddr::new(KERNEL_INFO.get().unwrap().physical_memory_offset),
    )
}
...
unsafe fn active_level_4_table() -> &'static mut PageTable {
    ...
    let virt = VirtAddr::new(KERNEL_INFO.get().unwrap().physical_memory_offset) + phys.as_u64();
